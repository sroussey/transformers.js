    ...
    )
{
    ...
    async _call_wav2vec2(audio, kwargs) {
        ...
        for (const aud of preparedAudios) {
            const inputs = await processor(aud);
    ...
    async _call_whisper(audio, kwargs) {
        ...
        const batchedAudio = single ? [audio] : audio;
        const processor = /** @type {any} */ (this.processor);
        const feature_extractor_config = processor.feature_extractor.config;
        ...
        for (const aud of preparedAudios) {
            ...
            if (chunk_length_s > 0) {
                ...
                while (true) {
                    ...
                    const feature = await /** @type {any} */ (processor)(subarr);
                ...
                chunks = [
                    {
                        ...
                        input_features: (await /** @type {any} */ (processor)(aud)).input_features,
                        is_last: true,
                    },
                ];
            }
    ...
    async _call_moonshine(audio, kwargs) {
        ...
        const toReturn = [];
        for (const aud of preparedAudios) {
            const inputs = await /** @type {any} */ (processor)(aud);
            ...
            const text = processor.batch_decode(/** @type {Tensor} */ (outputs), { skip_special_tokens: true })[0];
            toReturn.push({ text });
        }
        return single ? toReturn[0] : toReturn;
    }
