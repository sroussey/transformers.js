...
let ImageDataClass;
/** @type {((input: Blob | sharp.Sharp) => Promise<RawImage | ImageBitmap>) | undefined} */
...
export class RawImage {
    ...
    static async fromBlob(blob) {
        if (apis.IS_WEB_ENV) {
            ...
            const img = /** @type {ImageBitmap} */ (await loadImageFunction(blob));

            const ctx = /** @type {CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D} */ (createCanvasFunction(img.width, img.height).getContext('2d'));
            ...
            return /** @type {RawImage} */ (await loadImageFunction(img));
        }
    }
    ...
    async resize(width, height, { resample = 2 } = {}) {
        ...
        if (apis.IS_WEB_ENV) {
            ...
            const ctx = /** @type {CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D} */ (createCanvasFunction(width, height).getContext('2d'));
    ...
    async pad([left, right, top, bottom]) {
        ...
        if (apis.IS_WEB_ENV) {
            ...
            const ctx = /** @type {CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D} */ (createCanvasFunction(newWidth, newHeight).getContext('2d'));
    ...
    async crop([x_min, y_min, x_max, y_max]) {
        ...
        if (apis.IS_WEB_ENV) {
            ...
            const ctx = /** @type {CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D} */ (createCanvasFunction(crop_width, crop_height).getContext('2d'));
    ...
    toCanvas() {
        ...
        /** @type {CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D} */ (clonedCanvas.getContext('2d')).putImageData(data, 0, 0);
