...
async function clear_files_from_cache(modelId, files, options = {}) {
    ...
    const results = await Promise.all(
        files.map(async (filename) => {
            ...
            let deleted = false;
            if (wasCached) {
                const deletedWithProposed = await /** @type {Function} */ (cache.delete)(proposedCacheKey);
                const deletedWithLocal =
                    !deletedWithProposed && proposedCacheKey !== localPath ? await /** @type {Function} */ (cache.delete)(localPath) : false;
