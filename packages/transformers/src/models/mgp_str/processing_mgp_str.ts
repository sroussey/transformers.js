...
export class MgpstrProcessor extends Processor {
     ...
     */
    _decode_helper(pred_logits: import('../../utils/tensor.js').Tensor, format: 'char' | 'bpe' | 'wp') {
        if (!DECODE_TYPE_MAPPING.hasOwnProperty(format)) {
            throw new Error(`Format ${format} is not supported.`);
        }

        const [decoder_name, eos_token] = DECODE_TYPE_MAPPING[format];
        const decoder = (this as unknown as Record<string, Function>)[decoder_name as string].bind(this);

        const [batch_size, batch_max_length] = pred_logits.dims;
        const conf_scores = [];
        const all_ids = [];

        const pred_logits_list = pred_logits.tolist() as number[][][];
     ...
     */
    char_decode(sequences: number[][]) {
        return this.char_tokenizer.batch_decode(sequences).map((str: string) => str.replaceAll(' ', ''));
    }
     ...
     */
    wp_decode(sequences: number[][]) {
        return this.wp_tokenizer.batch_decode(sequences).map((str: string) => str.replaceAll(' ', ''));
    }
    ...
    batch_decode(...args: any[]): any {
        const [char_logits, bpe_logits, wp_logits] = args[0] as [import('../../utils/tensor.js').Tensor, import('../../utils/tensor.js').Tensor, import('../../utils/tensor.js').Tensor];
    ...
    static async from_pretrained(...args: Parameters<typeof Processor.from_pretrained>) {
        const base = await (super.from_pretrained as typeof Processor.from_pretrained)(...args);
    ...
    async _call(images: import('../../utils/image.js').RawImage | import('../../utils/image.js').RawImage[], text: string | string[] | null = null) {
