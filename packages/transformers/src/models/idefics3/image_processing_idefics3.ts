...
import { Tensor, cat, full, interpolate_4d, slice, stack } from '../../utils/tensor';
...
export class Idefics3ImageProcessor extends ImageProcessor {
    ...
    async _call(images: import('../../utils/image.js').RawImage | import('../../utils/image.js').RawImage[] | import('../../utils/image.js').RawImage[][], { do_image_splitting = null as boolean | null, return_row_col_info = false } = {}) {
        /** @type {RawImage[][]} */
        ...
        for (const image_batch of batched_2d_images) {
            let images_list = await Promise.all((image_batch as import('../../utils/image.js').RawImage[]).map((x: import('../../utils/image.js').RawImage) => this.preprocess(x)));

            // Original sizes of images
            original_sizes.push(...images_list.map((x: any) => x.original_size as [number, number]));

            // Reshaped sizes of images, before padding or cropping
            reshaped_input_sizes.push(...images_list.map((x: any) => x.reshaped_input_size as [number, number]));

            // Convert images to 4D tensors for easier processing
            images_list.forEach((x: { pixel_values: import('../../utils/tensor.js').Tensor }) => x.pixel_values.unsqueeze_(0));

            const { longest_edge } = this.max_image_size as { longest_edge: number };
            ...
            if (do_image_splitting ?? this.do_image_splitting) {
                ...
                images_tensor = await Promise.all(
                    images_list.map(async (x: { pixel_values: import('../../utils/tensor.js').Tensor }, i: number) => {
                        ...
                        const { frames, num_splits_h, num_splits_w } = await this.split_image(
                            resized as Tensor,
                            this.max_image_size as { longest_edge: number },
                        );
                        image_rows[i] = num_splits_h;
                        image_cols[i] = num_splits_w;
                        return cat(frames as Tensor[], 0);
                    }),
                );
                ...
                const size: [number, number] = [longest_edge, longest_edge];
                images_tensor = await Promise.all(images_list.map((x: { pixel_values: import('../../utils/tensor.js').Tensor }) => interpolate_4d(x.pixel_values, { size }))) as Tensor[];

                images_list_rows.push(new Array(images_list.length).fill(0));
                images_list_cols.push(new Array(images_list.length).fill(0));
            }

            all_pixel_values.push(cat(images_tensor as Tensor[], 0));
        }
        ...
        if (batch_size === 1) {
            ...
            for (let i = 0; i < batch_size; ++i) {
                ...
                if (num_patches < max_num_patches) {
                    ...
                    (pixel_attention_mask_data as unknown as Uint8Array).fill(0, start_offset, end_offset);
                }
            }
    ...
    async split_image(pixel_values: import('../../utils/tensor.js').Tensor, { longest_edge }: { longest_edge: number }) {
