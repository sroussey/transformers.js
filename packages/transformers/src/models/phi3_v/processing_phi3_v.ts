...
export class Phi3VProcessor extends Processor {
     ...
     * @returns {Promise<any>}
     */
    async _call(text: string | string[], images: RawImage | RawImage[] | null = null, { padding = true, truncation = true, num_crops = null as number | null } = {}) {
        ...
        if (images) {
            ...
            const num_img_tokens = image_inputs.num_img_tokens as number[];
            ...
            const prompt_chunks = text.map((t: string, i: number) =>
            ...
            (text_inputs.input_ids as import('../../utils/tensor').Tensor).map_((id: number) => (id == image_token_id ? -id : id));
        } else {
