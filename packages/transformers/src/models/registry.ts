...
for (const [mappings, type] of MODEL_CLASS_TYPE_MAPPING as [Map<string, string>, number][]) {
    for (const name of mappings.values()) {
        MODEL_TYPE_MAPPING.set(name, type);
        const model = (ALL_MODEL_FILES as Record<string, unknown>)[name];
...
for (const [name, model, type] of CUSTOM_MAPPING as [string, unknown, number][]) {
    MODEL_TYPE_MAPPING.set(name, type);
    MODEL_CLASS_TO_NAME_MAPPING.set(model, name);
    MODEL_NAME_TO_CLASS_MAPPING.set(name, model);
}
