...
class GreedySampler extends LogitsSampler {
    ...
    async sample(logits: Tensor) {
        ...
        const argmax = max(logits.data as Float32Array)[1];
...
class MultinomialSampler extends LogitsSampler {
    ...
    async sample(logits: Tensor) {
        ...
        // Get top k tokens
        const [v, i] = await topk(logits, k) as [Tensor, Tensor];

        // Compute softmax over logits
        const probabilities = softmax(v.data as Float32Array);

        return Array.from({ length: this.generation_config.num_beams }, () => {
            const sampledIndex = this.randomSelect(probabilities);
            return [
                (i.data as BigInt64Array)[sampledIndex], // token id
...
class BeamSearchSampler extends LogitsSampler {
    ...
    async sample(logits: Tensor) {
        ...
        return Array.from({ length: this.generation_config.num_beams }, (_, x) => {
            return [
                (i.data as BigInt64Array)[x], // token id
